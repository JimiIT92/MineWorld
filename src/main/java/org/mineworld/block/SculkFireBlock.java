...
/**
 * Implementation class for a {@link BaseFireBlock sculk fire}
 */
public class SculkFireBlock extends BaseFireBlock {
    ...
    public SculkFireBlock() {
        super(PropertyHelper.basicBlockProperties(MapColor.COLOR_BLUE, 0F, 0F, false, SoundType.WOOL)
                .replaceable()
                .noCollission()
                .instabreak()
                .lightLevel(state -> 7)
    ...
    public static boolean canSurviveOnBlock(final BlockState state) {
        return state.is(MWTags.Blocks.SCULK_FIRE_BASE_BLOCKS);
    }
